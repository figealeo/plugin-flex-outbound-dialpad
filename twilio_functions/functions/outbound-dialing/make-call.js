async function getCallTreatment(context, event) {
	...
	const callTreatmentUrl = encodeURI(
		...
		"/outbound-dialing/call-treatment?Token=" +
		event.Token +
		"&To=" +
		event.To
	);
...
function makeOutboundCall(context, event) {
	...
	return new Promise(async function (resolve, reject) {
		...
		const statusCallbackURL = encodeURI(
			"https://" +
			event.functionsDomain +
			"/outbound-dialing/call-status-update-handler?workerSyncDoc=" +
			event.workerContactUri + ".outbound-call" +
			"&workerSid=" + event.workerSid
		);

		const to = event.To;// await getCallTreatment(context, event);
